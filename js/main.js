document.addEventListener('DOMContentLoaded', () => {
  ...
  // — Bind standard offset menu items —
  ...
  fetch('data/drinks.json')
    ...
    .then(drinks => {
      ...
      drinks.forEach(d => {
        ...
        // extra tappable zone (opens popup)
    ...
    .catch(err => {
      console.error('Failed loading drinks.json:', err);
  ...
  // — Custom-Time 3-column picker —
  function openCustomPicker() {
    overlay.style.display = 'block';
    popup.classList.add('grid');
    popup.style.display = 'grid';

    // build 0–23 hours + four minutes buttons
  ...
  // — Log + toast/undo —
  function logDrinkWithTime(drink, btnOrItem, iso) {
    lastLog = {
