document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('touchend',   e => {
    ...
    const diff = e.changedTouches[0].screenX - touchStartX,
          curr = Math.round(parseFloat(wrapper.style.transform.replace(/[^0-9.-]/g,''))/100)||0;
    if (Math.abs(diff)>50) {
  ...
  fetch('drinks.json')
    ...
    .then(drinks => {
      const c = document.getElementById('drink-buttons');
      drinks.forEach(d => {
        ...
        btn.append(main, extra); c.appendChild(btn);
      });
      bindOffsetMenu();
    })
    .catch(err => {
      console.error('drinks.json load failed', err);
  ...
  function openCustomTimeGrid() {
    ...
    popup.innerHTML = ''; popup.style.display='grid';
    ...
    // minutes 00,15,30,45 col3 span 3 rows
  ...
  function logDrinkWithTime(d, btn, iso) {
    lastLog = { timestamp_logged: new Date().toISOString(),
                timestamp: iso,
    ...
    toast.onclick = ()=>{
      const h = JSON.parse(localStorage.getItem('drink_log')||'[]'),
            i = h.findIndex(x=>x.timestamp_logged===lastLog.timestamp_logged);
      if (i>-1){
        h.splice(i,1);
        localStorage.setItem('drink_log', JSON.stringify(h));
