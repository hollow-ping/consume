document.addEventListener('DOMContentLoaded', () => {
  ...
  function bindOffset() {
    popup.querySelectorAll('li').forEach(li => {
      li.onclick = e => {
        ...
        if (txt === 'Custom Time') {
          return openCustomGrid();
        }
        const mins = parseInt(txt, 10) || 0;
        const dt = new Date();
        dt.setMinutes(dt.getMinutes() - mins);
        logWithTime(lastDrink, lastBtn, dt.toISOString());
        closePopup();
      };
    });
  }

  // — load drinks.json (in /data/) —
    ...
    .then(list => {
      ...
      list.forEach(d => {
        ...
        main.onclick = () => {
          ...
          logWithTime(d, btn, new Date().toISOString());
        };
  ...
  function openCustomGrid() {
    ...
    // minutes 00,15,30,45 in col3, span 3 rows
      ...
      btn.onclick = e => {
        ...
        if (btn.dataset.minute != null && selH !== null) {
          ...
          setTimeout(() => {
            ...
            logWithTime(lastDrink, btn, dt.toISOString());
            closePopup();
          }, 200);
        }
      };
  ...
  // — log + toast/undo —
  function logWithTime(d, btn, iso) {
    lastLog = {
